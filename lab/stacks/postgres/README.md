...
To start with local testing run the `setup.sh` script to set local environmental variables for testing purposes.

```bash
./setup.sh
```

This should prompt you to set up the `$POSTGRES_USER` and `$POSTGRES_PASSWORD` environmental variables to access the DB
These are mounted as environmental variables in the local Docker container as a volume with mounted `env` variables.
